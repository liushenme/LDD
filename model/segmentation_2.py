...
class CrossModalFPNDecoder_flow(nn.Module):
    def __init__(self, feature_channels: List, conv_dim: int, mask_dim: int, dim_feedforward: int = 2048, norm=None):
        ...
        for idx, in_channels in enumerate(feature_channels):
            ...
            #audio_linear = nn.Linear(conv_dim, conv_dim)
            #self.add_module("linear_{}".format(stage), audio_linear)

            lateral_convs.append(lateral_conv)
            output_convs.append(output_conv)
            #audio_linears.append(audio_linear)
        # Place convs into top-down order (from low to high resolution)
        # to make the top-down computation in forward clearer.
        self.lateral_convs = lateral_convs[::-1]
        self.output_convs = output_convs[::-1]
        #self.audio_linears = audio_linears[::-1]

        mem_norm = get_norm(norm, conv_dim)
        self.mem_conv = Conv2d(
            conv_dim,
            conv_dim,
            kernel_size=3,
            stride=1,
        ...
        weight_init.c2_xavier_fill(self.mem_conv)
    ...
    def forward_features(self, features, features_flow, text_features, poses, memory, nf):
        # nf: num_frames
        ...
        #memory = rearrange(memory, '(b t) c h w -> b t c h w', b=memory.shape[0]//nf, t=nf)
        #for idx, (mem, f, pos) in enumerate(zip(memory[::-1], features[1:][::-1], poses[1:][::-1])): # 32x -> 8x
        #for idx, (f, pos) in enumerate(zip(features[1:][::-1].flip(dims=(0,)), poses[1:][::-1].flip(dims=(0,)))): # 32x -> 8x
        
        v_list = []
        a_list = []        

        for idx, (f, f_f, pos) in enumerate(zip(features[1:][::-1], features_flow[1:][::-1], poses[1:][::-1])): # 32x -> 8x
            lateral_conv = self.lateral_convs[idx]
            output_conv = self.output_convs[idx]
            cross_attn = self.cross_attns[idx]
            #audio_linear = self.audio_linears[idx]
            
            #text_features = audio_linear(text_features)
            #a_list.append(text_features)
            #print(text_features.shape)

            f_tensor, x_mask = f.decompose()
            f_f_tensor, x_mask = f_f.decompose()
            c_f_tensor = torch.cat((f_tensor, f_f_tensor), dim=1)

            pos = rearrange(pos, 'b t c h w -> (b t) c h w')
            n, c, h, w = pos.shape
            b = n // nf
            t = nf
            ...
            #print(memory.shape)
            if idx == 0:
                #f_tensor = torch.cat([f_tensor, memory], 1)
                #f_tensor = memory
                pass


            vision_features = lateral_conv(c_f_tensor)  # [b*t, c, h, w]

            #v_list.append(vision_features)
            ...
            #v_list.append(cur_fpn)

            # upsample
            if idx == 0: # top layer
                y = output_conv(cur_fpn)
                #x  = self.mem_conv(memory)
                #y = cur_fpn + F.interpolate(x, size=cur_fpn.shape[-2:], mode="nearest")
                #y = cur_fpn + x
                #y = output_conv(y)
            else:
        ...
        #audio_linear = self.audio_linears[-1]
        
        #text_features = audio_linear(text_features)
        #a_list.append(text_features)

        x, x_mask = features[0].decompose()
        f, f_mask = features_flow[0].decompose()
        c_x = torch.cat((x, f), dim = 1)
        pos = poses[0]
        pos = rearrange(pos, 'b t c h w -> (b t) c h w')
        n, c, h, w = pos.shape
        b = n // nf
        ...
        vision_features = lateral_conv(c_x)  # [b*t, c, h, w]
        ...
        y = output_conv(y)
        v_list.append(y)
        return y, v_list, a_list    # [b*t, c, h, w], the spatial stride is 4x

    def forward(self, features, features_flow, text_features, pos, memory, nf):
        ...
        """
        y, v_list, a_list = self.forward_features(features, features_flow, text_features, pos, memory, nf)
        return self.mask_features(y), v_list, a_list
...
class CrossModalFPNDecoder(nn.Module):
    ...
    def forward_features(self, features, text_features, poses, memory, nf):
        ...
        for idx, (f, pos) in enumerate(zip(features[1:][::-1], poses[1:][::-1])): # 32x -> 8x
            lateral_conv = self.lateral_convs[idx]
            output_conv = self.output_convs[idx]
            cross_attn = self.cross_attns[idx]
            #audio_linear = self.audio_linears[idx]
            ...
            vision_features = lateral_conv(f_tensor)  # [b*t, c, h, w]
    ...
    def forward(self, features, text_features, pos, memory, nf):
        ...
        Returns:
            mask_features (Tensor): [b*t, mask_dim, h, w], with the spatial stride of 4x.
        """
        y, v_list, a_list = self.forward_features(features, text_features, pos, memory, nf)
        return self.mask_features(y), v_list, a_list
